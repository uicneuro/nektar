...
namespace Nektar
{
    ...
    NekDouble FrankenHuxley::Computeghz(const NekDouble v, const NekDouble ci, const NekDouble co, const NekDouble Tc)
    {
        const NekDouble var_membrane__R = 8.3145; // gas constant: (J / mol / K)
        ...
        const NekDouble var_membrane_E = v + var_membrane__Vrest;

        NekDouble z, eco, eci, ghk;

        z = 1e-3 * var_membrane__F * var_membrane_E / ( var_membrane__R *  var_membrane__T ) ;
        eco = co * efun(z);
        eci = ci * efun(-1.0*z);

        ghk = 0.001 * var_membrane__F * ( eci - eco );
