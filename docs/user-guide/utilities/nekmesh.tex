...
\subsection{3D mesh scaling}
This module scales a 3D mesh in the $x$, $y$ and $z$ directions. It also works on
high-order meshes scaling the edge and face nodes. The module requires three parameters:

\begin{lstlisting}[style=BashInputStyle]
  NekMesh -m scale:scaleX=1.0:scaleY=2.0:scaleZ=2.0 mesh.xml mesh_scaled.xml
\end{lstlisting}

where \texttt{scaleX}, \texttt{scaleY} and \texttt{scaleZ} are the scaling
parameters in the $x$, $y$ and $z$ directions respectively. If not selected,
the scale parameters will be assumed 1.0 by default. 
...
The call is similar to the \emph{r}-adaption with interpolation file, with the difference of replacing the file with a list of CAD curve IDs, the scaling factor and an optional distance from the curve:
\begin{lstlisting}[style=BashInputStyle]
...
\subsection{Mesh Curving}
This module can take any linear mesh or high-order mesh, given that it is a close
representation of the CAD and project the boundary of the mesh onto the CAD.
This will reconstruct the connectivity between the CAD and the surface mesh.
Then it will curve and optimize the surface of the mesh. The method has a number
of fail-safes ensuring that even bad CAD or poor linear meshes should be able 
to be curved. If the method encounters an issue, such as the linear mesh being a
...
This module has been tested with hybrid prism-tet-pyramid, fully hexahedral and
tetrahedral meshes. As a best practice, it is recommended to use a single thick
O-type prismatic or hexahedral layer that is with sufficient thickness to account
for the mesh curving. 

The module needs to be informed of the CAD file to curve the mesh on the CAD and the
order at which to curve the surface:
\begin{lstlisting}[style=BashInputStyle]
  NekMesh -m projectcad:file=cadfile.step:order=x:cLength=y: inital.xml optimised.xml
\end{lstlisting}

The new version of the module reuses all state-of-the-art mesh curving modules with additional functionalities. 

\begin{itemize}
  \item \inlsh{surfopti}: True by default.Activates/Deactivates mesh curving and surface optimization. 
  \item \inlsh{ho}: Needs to be passed if the input mesh is already high-order.
  \item \inlsh{tolv1}: (optional) Tolerance for max distance of a node to CADSurface.
  \item \inlsh{tolv2}: (optional) Tolerance for max distance  (< tolv1) of a node to CADSurface.
  \item \inlsh{cLength}: (optional) Scales the tolerances by the factor, expands the
  bounding boxes and speeds up the bounding box generation. This is recommended
  to be tunned by the users need if the default values do not produce a good result.
  \item \inlsh{extract}: (optional) Exports the CAD-mesh connectivity to CAD.txt.
  \item \inlsh{cadtxt}:  (optional) Imports the CAD-mesh connectivity from CAD.txt directly.
\end{itemize}

Note: Please ensure that the STEP file is using the same dimension as the linear mesh.
A common error is to have a STP file in MiliMeters, whereas the mesh is in Meters or 
vice-versa. A way to check is to see the bounding box of printed in the beginning of
the module is in the correct order. 
